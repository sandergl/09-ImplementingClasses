...
class Point(object):
    ...
    def move_to(self, x, y):
        self.dt = self.dt + math.sqrt((self.x - x) ** 2 + (self.y - y) ** 2)
        self.x = x
        self.y = y
        self.move_count = self.move_count + 1
        return self.move_count and self.dt

    def move_by(self, dx, dy):
        self.dt = self.dt + math.sqrt((dx) ** 2 + (dy) ** 2)
        self.x = self.x + dx
        self.y = self.y + dy
        self.move_count = self.move_count + 1
        return self.move_count and self.dt
    ...
    def get_distance_from(self, p):
        ...
        self.dt = self.dt + a
        return a and self.dt

    def get_distance_from_start(self):
        a = math.sqrt((self.start_x - self.x) ** 2 + (self.start_y - self.y) ** 2)
        return a
